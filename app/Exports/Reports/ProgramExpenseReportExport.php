...
class ProgramExpenseReportExport implements FromCollection, WithColumnWidths, WithEvents, WithStyles, WithTitle
{
        ...
        if ($activities->isEmpty()) {
            ...
            FiscalYear::where('start_date', '<', $this->fiscalYear->start_date)
                ...
                ->each(function ($pastFy) use (&$cumulativeSpent) {
                    $cumulativeSpent += (float) ProjectExpenseFundingAllocation::where('project_id', $this->projectId)
            ...
            foreach ($capitalSectionSums as $key => $val) {
                $grandSums[$key] += $val;
            }
            foreach ($recurrentSectionSums as $key => $val) {
                $grandSums[$key] += $val;
            }
