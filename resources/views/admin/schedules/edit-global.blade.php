<x-layouts.app>
    ...
    <div class="container-fluid px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mt-1">
                        Update template #{{ $schedule->id }} • {{ $schedule->code }} • {{ $schedule->name }}
                    </p>
                </div>

                <a href="{{ route('admin.schedules.library') }}"
        ...
        <!-- Info Alert (now matches Create page style) -->
        <div
            ...
            <div class="flex items-start">
                ...
                <div>
                    ...
                    <div class="text-sm text-blue-700 dark:text-blue-400 space-y-2">
                        <p><strong>Project Type:</strong> Changing type may affect hierarchy validation</p>
                        <p><strong>Code:</strong> Be careful — changing code can break existing project references</p>
                        <p><strong>Parent:</strong> Cannot select self as parent</p>
                        <p>Changes affect <strong>future</strong> projects only</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="xl:col-span-2">
                <div class="bg-white dark:bg-gray-800 shadow-xl rounded-lg overflow-hidden">
                    <!-- Form Header – now matches Create -->
                    ...
                    <form action="{{ route('admin.schedules.library.update', $schedule->id) }}" method="POST"
                        class="p-6">
                        @csrf
                        @method('PUT')

                        <!-- Project Type Selection – now identical style to Create -->
                        <div
                            ...
                            <select name="project_type" id="project_type" required
                                ...
                                @foreach ($projectTypes as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('project_type', $schedule->project_type) == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="number" name="weightage" id="weightage"
                                value="{{ old('weightage', $schedule->weightage) }}" min="0" max="100"
                                step="0.01" placeholder="e.g., 25, 30, 15"
                        ...
                        <!-- Example Structures (same as Create) -->
                        <div
            ...
            <!-- Right Side: Existing Schedules Reference (header now matches Create) -->
                ...
                <div class="bg-white dark:bg-gray-800 shadow-xl rounded-lg overflow-hidden sticky top-6">
                    ...
                    <div id="schedules-list" class="p-4 max-h-[700px] overflow-y-auto">
                        <div class="text-center py-12 text-gray-400 dark:text-gray-500">
                            ...
                            <p class="text-sm italic">Loading schedules...</p>
                        </div>
                    </div>
    ...
    @push('scripts')
        ...
        <!-- Same drag & drop styles as Create -->
        <style>
            .sortable-ghost {
                opacity: 0.3;
        ...
        <script>
            ...
            function loadSchedulesForType(projectType) {
                ...
                schedules.forEach(schedule => {
                    if (schedule.id == currentScheduleId) return;

                    const option = document.createElement('option');
                    option.value = schedule.id;
                    option.textContent = `${schedule.code} - ${schedule.name}`;

                    if (schedule.id == {{ old('parent_id', $schedule->parent_id ?? 'null') }}) {
                ...
                schedules.forEach(schedule => {
                    ...
                    const bgClass = isCurrent ?
                        'bg-indigo-50 dark:bg-indigo-900/40 border-indigo-300 dark:border-indigo-600' :
                        'bg-gray-50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-600';

                    html += `
                    <div class="p-2.5 ${bgClass} rounded-lg border hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors cursor-move" data-id="${schedule.id}">
                        <div class="flex items-start" style="padding-left: ${indent}px;">
                            ...
                            <div class="ml-2 flex-1 min-w-0">
                                ...
                                ${isCurrent ? '<span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-indigo-100 text-indigo-800 dark:bg-indigo-800/40 dark:text-indigo-200 mt-1">Editing</span>' : ''}
                            </div>
                        </div>
                    </div>`;
                });
            ...
            document.getElementById('project_type').addEventListener('change', function() {
                loadSchedulesForType(this.value);
            });

            // Auto-suggest code (gentler version for edit)
            document.getElementById('parent_id').addEventListener('change', function() {
                const codeInput = document.getElementById('code');
                if (codeInput.value.trim() !== '') return; // don't overwrite

                if (!this.value) return;

                const parentCode = this.options[this.selectedIndex].text.split(' - ')[0];
                codeInput.value = parentCode + '.1';
                codeInput.focus();
            });

            // Code validation (same as Create)
            document.getElementById('code').addEventListener('input', function() {
                ...
                if (code) {
                    ...
                    if (pattern.test(code)) {
                        ...
                        feedback.textContent = '✓ Valid code format';
                    } else {
                        feedback.classList.add('text-yellow-600', 'dark:text-yellow-400');
                        feedback.textContent = '⚠ Format: A or A.1 or A.1.1 etc.';
                    }
            ...
            function saveOrder(schedules, projectType) {
                ...
                fetch('{{ route('admin.schedules.library.update-order') }}', {
                        method: 'POST',
                        headers: {
            ...
            function showToast(msg, type = 'success') {
                ...
                el.className = `fixed bottom-4 right-4 px-4 py-3 rounded-lg shadow-lg text-white text-sm font-medium z-50 transition-opacity duration-300 ${
            ...
            document.addEventListener('DOMContentLoaded', () => {
                const initialType = document.getElementById('project_type').value;
                if (initialType) {
                    loadSchedulesForType(initialType);
                }
            });
        </script>
    @endpush
