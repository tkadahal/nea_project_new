                ...
                <thead>
                    <tr class="bg-gray-200 dark:bg-gray-600">
                        ...
                        <th
                            ...
                            {{ strtoupper($currentQuarter) }} Quantity </th>
                        <th
                            class="border border-gray-300 dark:border-gray-600 px-2 py-1 text-sm font-semibold text-gray-700 dark:text-gray-200 text-right bg-blue-50 dark:bg-blue-900/20">
                            {{ strtoupper($currentQuarter) }} </th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    @foreach ($activities as $topActivity)
                        @php
                            ...
                            $topTotalQuantity = $hasTopChildren ? 0 : $topPlan?->total_quantity ?? 0;
                            $topTotalBudget = $hasTopChildren ? 0 : $topPlan?->total_budget ?? 0;
                            $topCompletedQuantity = $hasTopChildren ? 0 : $topPlan?->completed_quantity ?? 0;
                            $topTotalExpense = $hasTopChildren ? 0 : $topPlan?->total_expense ?? 0;
                            $topPlannedQuantity = $hasTopChildren ? 0 : $topPlan?->planned_quantity ?? 0;
                            $topPlanned = $hasTopChildren ? 0 : $topPlan?->planned_budget ?? 0;
                            $topQQuantity = $hasTopChildren ? 0 : $topPlan?->{$currentQuarter . '_quantity'} ?? 0;
                            $topQAmount = $hasTopChildren ? 0 : $topPlan?->{$currentQuarter . '_amount'} ?? 0;
                        @endphp
                        <tr class="projectActivity-row {{ $topBgClass }}" data-depth="0">
                            ...
                            <td
                                ...
                                {{ number_format($topQQuantity, 0) }} </td>
                            <td
                                class="border border-gray-300 dark:border-gray-600 px-2 py-1 text-right text-gray-700 dark:text-gray-200 bg-blue-50 dark:bg-blue-900/20">
                                {{ number_format($topQAmount, 2) }} </td>
                        </tr>
                        @if ($hasTopChildren)
                    ...
                    <tr
                        ...
                        <td class="border border-gray-300 dark:border-gray-600 px-2 py-1 text-left text-gray-900 dark:text-gray-100"
                            style="padding-left: 0px;"> Total {{ $header }} </td>
                        <td
                        ...
                        <td
                            ...
                            {{ number_format($grandQQuantity, 0) }} </td>
                        <td
                            class="border border-gray-300 dark:border-gray-600 px-2 py-1 text-right text-gray-900 dark:text-gray-100 bg-yellow-50 dark:bg-yellow-900/30">
                            {{ number_format($grandQAmount, 2) }} </td>
                    </tr>
                </tbody>
            </table>
        </div>
